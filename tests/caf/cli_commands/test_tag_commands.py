...
def test_tags_command(temp_repo: Repository, commit_hash: str, capsys: CaptureFixture[str]):
    ...
    cli_commands.create_tag(working_dir_path=temp_repo.working_dir, tag_name='v1.0', commit_hash=commit_hash, author='Some Author', message='Some Message')
    cli_commands.create_tag(working_dir_path=temp_repo.working_dir, tag_name='v2.0', commit_hash=commit_hash, author='Some Author', message='Some Message')
...
def test_create_tag_missing_args(temp_repo: Repository, commit_hash: str, capsys: CaptureFixture[str]):
    ...
    assert cli_commands.create_tag(working_dir_path=temp_repo.working_dir,
                                   tag_name=None, commit_hash=commit_hash, author='Some Author', message='Some Message') == -1
    assert 'Tag name is required' in capsys.readouterr().err
    
    assert cli_commands.create_tag(working_dir_path=temp_repo.working_dir,
                                   tag_name='v1.0', commit_hash=None, author='Some Author', message='Some Message') == -1
    assert 'Commit hash is required' in capsys.readouterr().err

    assert cli_commands.create_tag(working_dir_path=temp_repo.working_dir,
                                   tag_name='v1.0', commit_hash=commit_hash, author=None, message='Some Message') == -1
    assert 'Author is required' in capsys.readouterr().err

    assert cli_commands.create_tag(working_dir_path=temp_repo.working_dir,
                                   tag_name='v1.0', commit_hash=commit_hash, author='Some Author', message=None) == -1
...
def test_create_tag_nonexistent_hash(temp_repo: Repository, capsys: CaptureFixture[str]):
    ...
    assert cli_commands.create_tag(working_dir_path=temp_repo.working_dir,
                                   tag_name='v1.0', commit_hash=non_existent_hash, author='Some Author', message='Some Message') == -1
...
def test_delete_tag_command(temp_repo: Repository, commit_hash: str, capsys: CaptureFixture[str]):
    ...
    cli_commands.create_tag(working_dir_path=temp_repo.working_dir,
                            tag_name='v1.0', commit_hash=commit_hash, author='Some Author', message='Some Message')
    capsys.readouterr()  # Clear stdout
